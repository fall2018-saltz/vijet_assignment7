...
map1 <- ggplot(merged,aes(map_id=state_low)) + geom_map(map=usa,fill=merged$starea) + expand_limits(x=usa$long,y=usa$lat) + coord_map() #geom_map is used to plot a map using ggplot. Here we are using the variable 'usa' with data for states, thus telling R what map is to be produced. The 'fill' is to color code the map on the basis of the areas of states.
map1
#------------------------------------------------------------------------------------------------------
